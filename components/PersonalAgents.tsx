...
const PersonalAgents: React.FC<PersonalAgentsProps> = ({
    ...
    }, [chatMessages, viewMode, scrollToBottom]);
    ...
    const RenderChat = useMemo(() => (
        <div className="flex flex-col h-full bg-[#343541] animate-in fade-in duration-300">
            {/* 沉浸式顶部导航 */}
            <header className="h-16 border-b border-black/10 bg-[#343541]/95 backdrop-blur flex items-center justify-between px-6 shadow-sm z-10 sticky top-0">
                ...
                <div className="flex gap-2">
                    <button
                        ...
                        className="px-4 py-2 bg-red-600/10 hover:bg-red-600/20 text-red-400 hover:text-red-300 rounded-lg text-sm font-medium transition-colors flex items-center gap-2 border border-red-500/10"
                    >
                        <span>🗑️</span>
                        <span>清除会话</span>
                    </button>
                    <button
                        ...
                        className="px-4 py-2 bg-[#444654] hover:bg-[#505263] text-gray-200 rounded-lg text-sm font-medium transition-colors flex items-center gap-2 border border-white/5"
                    >
                        <span>✏️</span>
                        <span>配置</span>
                    </button>
            ...
            {/* 聊天区域 - 优化排版 */}
            <div className="flex-1 overflow-y-auto custom-scrollbar p-0 space-y-0 bg-[#343541] scroll-smooth">
                {chatMessages.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center opacity-40 select-none pb-20">
                        <div className="text-7xl mb-6 bg-[#444654] rounded-full p-8 shadow-inner border border-white/5">{editingAvatar}</div>
                        <h3 className="text-xl font-medium text-gray-300">How can I help you regarding {editingName}?</h3>
                    </div>
                ) : (
                    <div className="flex flex-col w-full items-center pb-40 pt-6">
                        {chatMessages.map((msg, i) => (
                            <div key={i} className={`w-full flex justify-center py-8 border-b border-black/5 ${msg.role === 'user' ? 'bg-[#343541]' : 'bg-[#444654]'}`}>
                                <div className="w-full max-w-4xl flex gap-6 px-4 md:px-6 relative group">
                                    <div className={`text-3xl shrink-0 select-none ${msg.role === 'user' ? 'order-2' : ''}`}>
                                        {msg.role === 'user' ? (
                                            <div className="w-9 h-9 rounded-md bg-[#5436DA] flex items-center justify-center text-sm text-white shadow-md">
                                                👤
                                            </div>
                                        ) : (
                                            <div className="w-9 h-9 rounded-md overflow-hidden flex items-center justify-center text-xl bg-[#19c37d] border border-black/10 shadow-md transform group-hover:scale-105 transition-transform">
                                                {editingAvatar}
                                            </div>
                                        )}
                                    </div>

                                    <div className={`flex-1 min-w-0 ${msg.role === 'user' ? 'order-1 text-right' : ''}`}>
                                        {/* Name header for better context */}
                                        <div className={`font-bold text-xs text-gray-400 mb-2 uppercase tracking-wide ${msg.role === 'user' ? 'text-right' : 'text-left'}`}>
                                            {msg.role === 'user' ? 'You' : editingName}
                                        </div>

                                        {/* Attachments */}
                                        {msg.attachments && msg.attachments.length > 0 && (
                                            <div className={`flex flex-wrap gap-3 mb-4 ${msg.role === 'user' ? 'justify-end' : ''}`}>
                                                {msg.attachments.map((img, idx) => (
                                                    <div key={idx} className="relative group/img overflow-hidden rounded-xl border border-white/10 shadow-lg">
                                                        <img
                                                            src={img}
                                        ...
                                        <div className={`text-gray-100 leading-8 text-[15px] md:text-[16px] tracking-wide ${msg.role === 'user' ? 'bg-[#40414f] text-left inline-block px-6 py-4 rounded-3xl rounded-tr-sm shadow-md' : 'prose prose-invert max-w-none prose-p:leading-8 prose-li:leading-8'}`}>
                                            {msg.role === 'user' ? (
                        ...
                        {isGenerating && (
                            <div className="w-full flex justify-center py-10 bg-[#444654] border-b border-black/5 animate-pulse">
                                <div className="w-full max-w-4xl flex gap-6 px-4 md:px-6">
                                    <div className="w-9 h-9 rounded-md overflow-hidden flex items-center justify-center text-xl bg-[#19c37d] border border-black/10 shadow-md shrink-0">
                                        {editingAvatar}
                                    </div>
                                    <div className="flex items-center gap-1 h-9">
                                        <div className="w-2.5 h-2.5 bg-gray-400/80 rounded-full animate-bounce"></div>
                                        <div className="w-2.5 h-2.5 bg-gray-400/80 rounded-full animate-bounce" style={{ animationDelay: '0.1s' }}></div>
                                        <div className="w-2.5 h-2.5 bg-gray-400/80 rounded-full animate-bounce" style={{ animationDelay: '0.2s' }}></div>
                                    </div>
                                </div>
                            </div>
                        )}
            ...
            {/* 输入区域 */}
            <div className="relative bg-gradient-to-t from-[#343541] via-[#343541] to-transparent pt-12 pb-8 px-4">
